...
async function fetchUnoptimized(
  ...
  if (sourceUrl.startsWith("/")) {
    ...
    return fetch(resolveWorkerUrl(address, sourceUrl));
  }
...
export function createVercelImageHandler(opts: {
  ...
  return async function handleVercelImage(request: Request): Promise<Response> {
    ...
    if (!ipx) {
      return fetchUnoptimized(sourceUrl, getAddress);
    }
    ...
    try {
      ...
      if (config?.contentDispositionType) {
        ...
        headers["content-disposition"] = `${config.contentDispositionType}; filename="${filename}"`;
      }
