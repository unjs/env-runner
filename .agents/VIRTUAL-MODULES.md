...
- `unsafeModuleFallbackService` serves virtual keys **before** any other resolution, overriding real files and `transformRequest`. It matches `rawSpecifier`, `specifier` and `specifier` without its leading `/` verbatim, with the query stripped, then path keys by path (see below): `rawSpecifier` resolved against the referrer's real path first (needed on Windows, see [`MINIFLARE.md`](MINIFLARE.md)), then `specifier`. Verified: workerd passes `#`-prefixed specifiers intact.
