...
- **No proxy/worker file** — `fetch()` calls `mf.dispatchFetch()`. The wrapper is passed as in-memory `script` with `scriptPath` = `<entryDir>/__env_runner_wrapper.mjs` and `modulesRoot: "/"`.
...
- **Module resolution** — the fallback service serves `data.virtual` keys first, then `transformRequest` (`{ code } | null`), then disk. The wrapper is always generated `dynamicOnly` (no static `export *`) so Miniflare's ModuleLocator doesn't pre-walk the import tree; with `transformRequest`, `.ts/.tsx/.jsx/.mts` `modulesRules` are added so the locator doesn't reject them.
- **Exports / DO auto-wiring** (opt-in `exports`) — detected classes become `SCREAMING_SNAKE` DO bindings unless the name is taken or the class is already bound locally (string or object without `scriptName`), merging with wrangler/user bindings. Named exports + a virtual entry throw: the static re-exports would be resolved on disk at startup. DO/Entrypoint classes receive workerd's raw `env`.
- **IPC** — during init a `dispatchFetch` with `upgrade: "websocket"` creates a persistent WebSocket pair carrying all JSON messages (user messages, reload, shutdown). A WebSocket from one request context can't be used from another in workerd, so messages sent during `fetch` handling go through the `__ENV_RUNNER_IPC` service binding instead.
...
Enabling `wrangler` (any form, whether or not a file loaded) defaults `defaultPersistRoot` to `<dir>/.wrangler/state/v3` — what `wrangler dev` uses, so state is shared. `<dir>` = loaded config file's dir, else the requested path's dir (even if missing), else cwd. Skipped when `miniflareOptions` sets `defaultPersistRoot` or any `*Persist` key.

## Testing

- `test/miniflare.test.ts` — DO exports, IPC, `reloadModule()`, explicit `miniflare` option, srvx cloudflare context (`test/fixtures/worker-srvx.mjs`; response headers are decorated because workerd request headers are immutable). `test/miniflare-request.test.ts` — `dispatchFetch` request fidelity. Virtual modules → `test/virtual.test.ts`.
- `test/wrangler.test.ts` — matrix over both backends (package / minimal reader) using a capturing `Miniflare` subclass, plus a fast `loadWranglerConfig()`-only block (discovery, self-referencing DOs, warnings dedupe, dev vars/`envFiles`, `no_nodejs_compat`).
- Gotchas:
  - Minimal-reader cases must pass `wranglerModule: false` — omitting it would optional-import the installed `wrangler`.
  - Miniflare creates persist dirs eagerly, so the capturing subclass strips a `defaultPersistRoot` outside the temp dir before `super()`; cwd-anchored cases assert on captured options only (no `.wrangler/` in the repo root).
