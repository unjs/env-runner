...
- **Module resolution** — the fallback service serves `data.virtual` keys first, then `transformRequest` (`{ code } | null`), then disk. The wrapper is always generated `dynamicOnly` (no static `export *`) so Miniflare's ModuleLocator doesn't pre-walk the import tree; with `transformRequest` on v4, `.ts/.tsx/.jsx/.mts` `modulesRules` are added so the locator doesn't reject them.
- **Exports / DO auto-wiring** (opt-in `exports`) — detected classes become `SCREAMING_SNAKE` DO bindings unless the name is taken or the class is already bound locally (string or object without `scriptName`), merging with wrangler/user bindings. A string re-exports a separate module (disk or virtual), with bindings configured explicitly. Named exports use an explicit `modules` array for the wrapper so the fallback service resolves their imports at startup, including virtual entries. Recreate the runner when classes change. DO/Entrypoint classes receive workerd's raw `env`.
