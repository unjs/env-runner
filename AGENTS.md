...
- **`test/virtual.test.ts`** — Tests virtual modules for `NodeWorkerEnvRunner`, `NodeProcessEnvRunner`, `BunProcessEnvRunner` (auto-skipped when `bun` is not installed), and `DenoProcessEnvRunner` (auto-skipped when `deno` is not installed) via the `data.virtual` map: a virtual import, a virtual entry (`data.entry` = a virtual key), a virtual entry composing another virtual module, TypeScript virtual modules (`.ts` entry + import on Node/Bun; fail-fast on Deno), JSON virtual modules (attribute-free on all runtimes, `with { type: "json" }` on Node/Bun only), factory-valued sources (sync and async), reload of a virtual entry, graceful worker shutdown with virtual modules registered, and (via the `virtual-unregister.mjs` subprocess fixture) unregistration on both the registerHooks and Bun.plugin backends
