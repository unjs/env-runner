...
- **`test/wrangler.test.ts`** — Tests `MiniflareEnvRunner`'s `wrangler` option via the installed `wrangler` package: loading `vars` from a `wrangler.jsonc` (explicit path), auto-discovery next to the entry (`wrangler: true`), `--env` selection (`wranglerEnv`), an **inline config object** (plus inline `--env`), and `miniflareOptions.bindings` winning over wrangler `vars`. (Configs pin a `compatibility_date` so workerd accepts them regardless of the system clock.)
- **`test/wrangler-fallback.test.ts`** — Tests the built-in minimal config reader by `vi.mock`ing `wrangler` to throw on import (simulating it not being installed): plain-JSON `vars` reach the worker with a one-time "not installed" warning, an **inline config object** maps via the minimal mapper, and JSONC/TOML files are skipped with a warning
