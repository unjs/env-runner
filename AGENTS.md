...
- **`test/virtual.test.ts`** — Tests virtual modules for `NodeWorkerEnvRunner`, `NodeProcessEnvRunner`, `BunProcessEnvRunner` (auto-skipped when `bun` is not installed), `DenoProcessEnvRunner` (auto-skipped when `deno` is not installed), and `MiniflareEnvRunner` via the `data.virtual` map: a virtual import, a virtual entry (`data.entry` = a virtual key), a virtual entry composing another virtual module, TypeScript virtual modules (`.ts` entry + import on Node/Bun/Miniflare/Deno-with-`stripTypeScriptTypes`; fail-fast on older Deno — detected by probing `deno eval` at collection time), JSON virtual modules (attribute-free on all runtimes, `with { type: "json" }` on Node/Bun only — workerd rejects import attributes), factory-valued sources (sync and async), reload of a virtual entry, graceful worker shutdown with virtual modules registered (skipped on miniflare — no shutdown handshake), fail-fast for named `exports` with a virtual entry on miniflare, and (via the `virtual-unregister.mjs` subprocess fixture) unregistration on both the registerHooks and Bun.plugin backends
